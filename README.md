...
- 题目报错与运营处理：题目卡片一键报错（问题类型 + 补充说明），同一账号同题未处理时返回原编号不重复生成；提交后卡片显示处理中，原题和解析照常练习；运营标记已修正/无需修改并填写处理说明后，用户再做到该题即可看到处理结果。
...
- `POST /reports/`：提交报错（需登录），参数 `question_id`、`issue_type`（`stem`/`answer`/`explanation`/`other`）、`note`；同一账号对同一题已有处理中报错时返回原编号（`duplicated: true`），不重复生成。
- `GET /reports/mine/`：当前用户的报错列表，含处理状态和处理说明。
- `GET /reports/pending/`：运营（staff）查看全部待处理报错。
- `POST /reports/<id>/resolve/`：运营处理，参数 `result`（`fixed`/`nochange`）和必填的 `note`（处理说明）。
- 演示账号：页面右上角「演示登录」为普通用户，「运营登录」为运营账号。
