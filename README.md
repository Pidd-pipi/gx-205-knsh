...
运营处理台入口在前端右上角，默认运营账号 `ops` / `ops123456`（可通过 `OPS_USERNAME` / `OPS_PASSWORD` 覆盖，后端启动时幂等创建）。普通用户可用「演示登录」按钮获取练习账号。
...
- 题目报错：题目卡片可一键报错，选择问题类型（题干/答案/解析/其他）并填写补充说明；同一账号对同一道题在上一条未处理时返回原报错编号，不重复生成；处理中题目与解析照常作答。运营在「运营处理台」标记已修正或无需修改并填写处理说明后，用户再次做到该题即可看到处理结果。
...
| `OPS_USERNAME` | 运营账号用户名（处理题目报错） | `ops` |
| `OPS_PASSWORD` | 运营账号密码 | `ops123456` |
